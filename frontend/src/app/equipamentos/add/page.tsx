...
export default function AddEquipamentoPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/utils/equipments`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ descricao, numero_serie: numeroSerie }),
      });
      const data = await res.json();
      if (!res.ok) {
        setErro(data.error || "Erro ao adicionar equipamento");
      } else {
        setSucesso("Equipamento adicionado com sucesso!");
        setDescricao("");
        setNumeroSerie("");
      }
    } catch {
  ...
  return (
    <div className="mx-auto flex w-full max-w-md flex-col gap-6 rounded-2xl border border-white/10 bg-slate-950/60 p-6 shadow-2xl shadow-emerald-500/10">
      <h1 className="text-2xl font-semibold text-slate-50">Adicionar Equipamento</h1>
      <form onSubmit={handleSubmit} className="space-y-4 text-sm">
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-200">Número de Série</label>
          <input
            type="text"
            className="w-full rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2 text-sm text-slate-50 outline-none transition focus:border-emerald-400 focus:ring-1 focus:ring-emerald-400"
            value={numeroSerie}
